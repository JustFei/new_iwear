...
- (void)createUI
{
    ...
    [tipLabel setText:NSLocalizedString(@"updateTips", nil)];   //@"升级中请勿使设备远离手机"];
...
- (void)otaWithFile
{
    ...
    //1.创建 DFU 对象
    DFUServiceInitiator *initiator = [[DFUServiceInitiator alloc] initWithCentralManager: [BleManager shareInstance].myCentralManager target:[BleManager shareInstance].currentDev.peripheral];
    //2.选择刷入的固件
...
- (void)dfuStateDidChangeTo:(enum DFUState)state
{
    switch (state) {
        ...
        case DFUStateCompleted:
        {
            ...
            [BleManager shareInstance].myCentralManager.delegate = [BleManager shareInstance];
            [[BleManager shareInstance] connectDevice:[BleManager shareInstance].currentDev];
...
- (void)dfuProgressDidChangeFor:(NSInteger)part outOf:(NSInteger)totalParts to:(NSInteger)progress currentSpeedBytesPerSecond:(double)currentSpeedBytesPerSecond avgSpeedBytesPerSecond:(double)avgSpeedBytesPerSecond
{
    ...
    self.currentLabel.text = [NSString stringWithFormat:@"%@%ld%%", NSLocalizedString(@"complete", nil), (long)progress];
}
...
- (UILabel *)currentLabel
{
    if (!_currentLabel) {
        ...
        [_currentLabel setText:[NSString stringWithFormat:@"%@0%%", NSLocalizedString(@"complete", nil)]];
